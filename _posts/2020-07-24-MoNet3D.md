...
1. 2D Detection，把原始图像划分成 32x32 的grids，用 YOLO 检测，每个 `grid` 产生两个bbox（？？不懂为什么么限定这个），每个bbox表示为 $b_{2d}^g = (u^{(2d)}, v^{(2d)}, d, h)$ 以及 $Pr_{obj}$，b_{2d}^g 的四个量分别代表中心点横纵坐标、高、宽，然后用 NMS + ROIAlign，这部分的损失函数表示为
    ...
    $$ L_{conf} = \mathfrak{E}_{g} [\mathfrak{S}(\hat){pr}_{obj}^{g}), Pr_{obj}^{g}] $$
    $$ L_{b2d} = \sum_{g}_{g}^{obj} \cdot \mathfrak{L}_{1}(\hat{b}_{2d}^{g}, b_{2d}^{g})$$
    $ \mathfrak{S}(\cdot)$ 是 softmax fuction
    $ \mathfrak{E}(\cdot)$ 是 cross entropy fuction
    $\mathfrak{L}_{1}$ 是 L1 distance
    $\alpha$ 是平衡因子
2. Car Depth Estimation，$z_{g}$ 代表某个 `grid` 中的 ground truth 物体深度，估计物体深度时，先用 deep features 预测一个大概的深度 $z_{coa}^{g}$，再用 shallow features 产生的预测 $z_{\delta}^{g}$ 作调整，这部分的损失函数表示为
    $$ L_{z} = \gamma L_{zcoa} + L_{\delta z} $$
    $$ L_{zcoa} = \sum_{g}_{g}^{obj} \cdot (\hat{z}_{coa}^{g}, z^{g}) $$
    $$ L_{\delta z} = {\sum_{g}}_{g}^{obj} \cdot (\hat{z}_{coa}^{g} + z_{\delta}^{g}, z^{g}) $$
    $\delta$ 是平衡因子
3. 3D Box Estimation，预测3D中心点 $C_{3d} = (u^{(3d)}, v^{(3d), z^{(3d)},)$ 和 立方体八个顶点 $\mathfrak{O} = {\textbf{O}_{k=1}^8}$（8个顶点用 deep features 回归估计）。2D Box 的中心点 $C_{2d} = (u^{(2d)}, v^{(2d)})$，根据成像模型，像素坐标和相机坐标的关系为：
    ![](../img/post/pixel2cam.png)
    - $f$ 是焦距，$\theta$、$\phai$ 是成像平面中心点横纵坐标
    3D中心点损失函数为：
