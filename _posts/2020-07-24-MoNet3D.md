...
3. 3D Box Estimation，预测3D中心点 $C_{3d} = (u^{(3d)}, v^{(3d), z^{(3d)},)$ 和 立方体八个顶点 $\mathcal{O} = {\textbf{O}_{k=1}^8}$（8个顶点用 deep features 回归估计）。2D Box 的中心点 $C_{2d} = (u^{(2d)}, v^{(2d)})$，根据成像模型，像素坐标和相机坐标的关系为：
